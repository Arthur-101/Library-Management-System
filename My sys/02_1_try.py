...
def student_return_books(entry_bid):
    bid_value = entry_bid.get()
    sql_return_books = f"""SELECT * FROM {book_issue_detail} WHERE bid = ? AND username = {info_username};"""
    cur.execute(sql_return_books, (bid_value,))
    return_books = cur.fetchall()

    if return_books:
        sql_update_books = f"""UPDATE {books_detail} SET status = 'Available' WHERE bid = ? AND status = 'Unavailable';"""
        cur.execute(sql_update_books, (bid_value,))

        sql_delete_borrowed = f"""DELETE FROM {book_issue_detail} WHERE bid = ? AND username = {info_username};"""
        cur.execute(sql_delete_borrowed, (bid_value,))
