...
def student_main_win_allbooks():
    # global student_tree_frame_allbooks, filter_frame
    # Remove all the widgets from the main_second_frame_student_win
    try:
        for widget in main_second_frame_student_win.winfo_children():
            widget.destroy()
...
def filter_down_allbooks(tree_frame, filter_frame):
    ...
    ###   Filter Keys   ###
    heading_filter = CTkOptionMenu(filter_frame, values=['bid','title','subject','author', 'status'])
    heading_filter.place(relx=0.05, rely=0.15, relwidth=0.1, relheight=0.2)
    
    radio_var_1 = tk.StringVar(filter_frame, value='ascending')
    radio_button_1 = CTkRadioButton(filter_frame, variable=radio_var_1, value='ascending', text='Ascending', font=('Roboto',16,))
    radio_button_1.place(relx=0.3, rely=0.15, relwidth=0.1, relheight=0.15)
    radio_button_2 = CTkRadioButton(filter_frame, variable=radio_var_1, value='descending', text='Descending', font=('Roboto',16,))
    radio_button_2.place(relx=0.3, rely=0.35, relwidth=0.1, relheight=0.15)
