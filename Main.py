...
def student_main_win_profile(username):
    ...
    profile_delete_button = CTkButton(profile_frame,text='Delete Profile',border_width=2,corner_radius=5,border_color='#ff0000',
        fg_color='#ff3737', hover_color='#d20000',font=('Roboto',22,),command=lambda: student_profile_delete(name_entry))
    profile_delete_button.place(relx=0.8,rely=0.85, relwidth=0.15, relheight=0.1)
...
def student_profile_delete(name_entry):
    name_entry.configure(state="normal")
    user = name_entry.get()
    name_entry.configure(state="disabled")
    
    confirm = CTkMessagebox(title="Confirmation", message=f"All Books will be returned\nand your account will be DELETED\nAre You sure about this?", icon='info', option_1='No', option_2='Yes', font=("Arial", 15))
    if confirm.get() == 'Yes':
        ...
        ### Delete Username
        sql_student_username = f"DELETE FROM {student_detail} WHERE username = ?"
        cur.execute(sql_student_username, (user,))
...
def admin_main_win_profile(adminname):
    ...
    profile_delete_button = CTkButton(profile_frame,text='Delete Profile',border_width=2,corner_radius=5,border_color='#ff0000',
        fg_color='#ff3737', hover_color='#d20000',font=('Roboto',22,),command=lambda: admin_profile_delete(name_entry))
    profile_delete_button.place(relx=0.8,rely=0.85, relwidth=0.15, relheight=0.1)
...
def admin_profile_delete(name_entry):
    name_entry.configure(state="normal")
    admin = name_entry.get()
    name_entry.configure(state="disabled")
    
    confirm = CTkMessagebox(title="Confirmation", message=f"Your account will be DELETED\nAre You sure about this?", icon='info', option_1='No', option_2='Yes', font=("Arial", 15))
    if confirm.get() == 'Yes':
        ### Delete Username
        sql_admin_username = f"DELETE FROM {admin_detail} WHERE admname = ?"
        cur.execute(sql_admin_username, (admin,))
